...
\myattach{ApproximatePi.sq}
\myattach{L03ApproximatePi.png}
\myattach{L03L2NormCtrl.png}
\myattach{UnitMassSystemControl.sq}
\vskip 1cm
...
\begin{solution-c26e158309505f61889ee3b749c66e08}
 ...
 \\4:To solve $J=min\left\lVert v \right\rVert=min \left\lVert Ax-y  \right\rVert=min\sqrt{\sum_{i=1}^{n}(a_{i}^{T}x-y_{i})^{2}}$, assume 
 $$L=\sum_{i=1}^{n}(a_{i}^{T}x-y_{i})^{2}$$
 $$\frac{\partial L}{\partial x}=\sum_{i=1}^{n}(2a_{i}a_{i}^{T}x-2y_{i}a_{i})=0$$, then we obtain the optimal solution $x_{ls}=(\sum_{i=1}^{n}a_{i}a_{i}^{T})^{-1}\sum_{i=1}^{n}y_{i}a_{i}$
...
Minimize $\left\lVert x \right\rVert_{2}$ s.t $Ax=y$ by minimizing
$$J=\left\lVert x \right\rVert_{2}^{2}+\lambda^{T}(y-Ax)$$
$$0=\frac{\partial J}{\partial x}=2x-A^{T}\lambda$$
$$x=\frac{1}{2}A^{T}\lambda$$
Given
$$\frac{\partial J}{\partial \lambda}=0$$
which is equivalent to $y-Ax=0$, we obtain:
$$y=Ax=\frac{1}{2}AA^{T}\lambda$$
$$\lambda=2(AA^{T})^{-1}y$$
Hense,
$$x_{l2n}=A^{T}(AA^{T})^{-1}y$$. 

4:Now we plot the least 2-norm solution by running command: 

\begin{verbatim}
x=Least2NormControl(1,0,4)
DisplayMotion(x)
norm(x,2)

\end{verbatim}
We obtain the least 2-norm control and the states in each time interval. 
\begin{verbatim}
x =
       0.3
      1e-1
...
\begin{figure}[h!]
\includegraphics[width=\linewidth]{L03L2NormCtrl.png}
\end{figure}
\\5:It's obvious that $n\geq2$ otherwise there's no solution. 
Denote as $\left\lVert A\right\rVert_{p}$ the induced matrix p-norm. 
We have $\left\lVert y\right\rVert_{\infty}=\left\lVert Ax\right\rVert_{\infty}\leq\left\lVert A\right\rVert_{\infty}\left\lVert x\right\rVert_{\infty}$. Thus, we have $max\{|f_{1}|, |f_{2}|, \cdots, |f_{n}|\}=\left\lVert x\right\rVert_{\infty}\geq \frac{\left\lVert y\right\rVert_{\infty}}{\left\lVert A\right\rVert_{\infty}}=J$. In case $n\geq2$, $J=\frac{\left\lVert y\right\rVert_{\infty}}{\left\lVert\alpha_{1}\right\rVert_{1}}$ where $\alpha_{1}$ is a vector form by the elements in the first row of A. Given that $\left\lVert\alpha_{1}\right\rVert_{1}$ is sum of arithmetic sequence we have $J=\left\lVert\alpha_{1}\right\rVert_{1}=\frac{n^{2}}{2}$
If the intersection of $\{x|y=Ax\}$ and $\{\left\lVert x\right\rVert_{\infty}=J\}$  is not empty, the problem is equivalent to:
Solve $y=Ax$ s.t $\left\lVert x\right\rVert_{\infty}=J$ 
\\6:
\\
\\Finally, the modified code is shown below:
